...
class Button:
        ...
        while not self._done.is_set():
            ...
            if GPIO.input(self._channel) == self._expected:
                if pressed == False:
                    when_pressed = now
                    pressed = True
                if (now - when_pressed > self._debouncelong_time) and (longpressed == False) :
                    logging.info('Board - long pressed '+str(now - when_pressed))
                    self._trigger(self._longpressed_queue, self._longpressed_callback)
                    longpressed = True
            else:
                if (now - when_pressed > 0) and (pressed == True) and (now - when_pressed <= self._debouncelong_time) :
                    logging.info('Board - short pressed '+str(now - when_pressed))
                    self._trigger(self._pressed_queue, self._pressed_callback)
                    logging.info('Board - released '+str(now - when_pressed))
    ...
    def _run2(self):
        ...
        while not self._done.is_set():
            ...
            if now - when_pressed > self._debounce_time:
                if GPIO.input(self._channel) == self._expected:
                    logging.info('debug - when_pressed '+str(when_pressed))
                    if not pressed:
                        pressed = True
                        when_pressed = now
                        self._trigger(self._pressed_queue, self._pressed_callback)
    ...
    def __init__(self, channel, edge='falling', pull_up_down='up',
                 debounce_time=0.08, debouncelong_time=1):
    ...
    def _when_longpressed(self, callback):
        self._longpressed_callback = callback
    when_longpressed = property(None, _when_longpressed)
    """A function to run when the button is longpressed."""
    ...
    def wait_for_press(self, timeout=None):
        ...
        self._longpressed_queue.put(event)
